<!DOCTYPE html>
<html lang="en" dir="ltr">
  <head>
    <meta charset="utf-8">
    <title>TKDS Notification Mail</title>
  </head>
  <body>

    <p>
        Дорогой друг:)<br /><br />

        Мы рады видеть вашу заявку на регистрацию в танцевальную студию TKDS.<br />
        Для полной регистрации необходимо заполнить и отправить подписанный договор.<br />
        Договор который нужно заполнить и отправить прикреплён к данному письму.<br />

        (NB! Документ необходимо выслать ДО начала первого занятия)<br /><br />

        Следите за нашими страницами в Facebook/instagram (tkds_studio). Туда будем помещать важную информацию касаемую изменений, мероприятий.<br />

        Наш адрес: Tatari 64 (2-ой этаж)<br /><br />

        <a href="https://tkds.ee/#dancestyle">Стили</a>
        <a href="https://tkds.ee/#timetable">Расписание</a>
        <a href="https://tkds.ee/hinnakiri">Цены</a><br /><br />

      ______________________________________
    </p>

    <p>
        Armas tantsusõber,<br /><br />

        Olete täitnud TKDS Dance Studio registreerimisvormi.<br /><br />

        Täielikuks registreerimiseks peate täitma ja saatma allkirjastatud lepingu.<br />
        (NB! Dokument peab olema tagasi allkirjastatuna saadetud enne esimest trenni)<br />
        Täidetav ja saadetav leping on lisatud käesolevale kirjale.<br />

        Jälgige meie uudiseid, trennidega seotuid muudatusi ja tulevasi üritusi meie Facebook ja Instagram lehelt (tkds_studio).<br />

        Meie aadress: Tatari 64 (2 korrus)<br /><br />

        <a href="https://tkds.ee/#dancestyle">Tantsustiilid</a>
        <a href="https://tkds.ee/#timetable">Tunniplaan</a>
        <a href="https://tkds.ee/hinnakiri">Hinnakiri</a><br />
    </p>

    <p>
      Lugupidamisega<br />
      TKDS STUDIO<br />
      tel: +372 56669328<br />
      www.tkds.ee
    </p>

  </body>
</html>
